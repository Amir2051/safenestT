...
Return JSON with validation result.`,
      ...
      if (!acrisValidation.is_valid && !isAdmin) {
...
Return JSON with status.`,
      ...
      if (acrisCheck.suspicious_activity && !isAdmin) {
  ...
  return (
    <>
      <Card className={`bg-gradient-to-br from-[#1a2332] to-[#0f1419] border-2 transition-all ${
        isLocked ? 'border-green-500/50 shadow-lg shadow-green-500/20' : 'border-gray-700'
      }`}>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          {isLocked ? (
            <>
              <div className="p-5 bg-green-500/10 border-2 border-green-500/20 rounded-xl">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 bg-green-500/20 rounded-xl flex items-center justify-center">
                    <CheckCircle className="w-7 h-7 text-green-400" />
                  </div>
                  <div>
                    <p className="text-white font-bold text-lg">Property Locked</p>
                ...
                {activeLock && (
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between p-2 bg-[#0f1419] rounded">
                      <span className="text-gray-400">Locked since:</span>
                      <span className="text-white font-semibold">
                        {new Date(activeLock.locked_at).toLocaleDateString()}
                      </span>
                    </div>
                    {activeLock.lock_expiry && (
                      <div className="flex justify-between p-2 bg-orange-500/10 rounded border border-orange-500/20">
                        <span className="text-gray-400">⏰ Auto-expires:</span>
                        <span className="text-orange-400 font-semibold">
                          {new Date(activeLock.lock_expiry).toLocaleDateString()}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between p-2 bg-[#0f1419] rounded">
                    ...
                    {activeLock.lock_bypass_attempts && activeLock.lock_bypass_attempts.length > 0 && (
                      <div className="p-3 bg-red-500/10 border border-red-500/20 rounded mt-3">
                        <p className="text-red-400 font-bold text-sm">
                          ⚠️ {activeLock.lock_bypass_attempts.length} Bypass Attempt{activeLock.lock_bypass_attempts.length > 1 ? 's' : ''} Blocked
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="p-4 bg-cyan-500/10 border border-cyan-500/20 rounded-lg">
                <p className="text-sm text-cyan-300 mb-3 font-semibold">
                  <strong>🛡️ Active Protection:</strong>
                </p>
                <ul className="text-xs text-gray-300 space-y-2">
                  <li className="flex items-center gap-2">
                    <CheckCircle className="w-3 h-3 text-green-400" />
                    Blocks unauthorized ownership changes
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle className="w-3 h-3 text-green-400" />
                    Prevents new liens or mortgages
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle className="w-3 h-3 text-green-400" />
                  ...
                  <li className="flex items-center gap-2">
                    <CheckCircle className="w-3 h-3 text-green-400" />
                    Alerts you of bypass attempts
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle className="w-3 h-3 text-green-400" />
                    Real-time NYC ACRIS validation
                  </li>
                </ul>
              </div>
              ...
              <Button
                onClick={() => {
                  if (isAdmin) {
                    setUnlockReason('Admin override');
                    setUnlockCode('ADMIN');
                    verifyUnlockMutation.mutate();
                  } else {
                    requestUnlockMutation.mutate();
                  }
                }}
                disabled={requestUnlockMutation.isPending || verifyUnlockMutation.isPending}
                variant="outline"
                className="w-full border-orange-500/20 text-orange-400 hover:bg-orange-500/10"
              >
                {requestUnlockMutation.isPending || verifyUnlockMutation.isPending ? (
                  <>
                    <Mail className="w-4 h-4 mr-2 animate-pulse" />
                    {isAdmin ? 'Unlocking...' : 'Sending Verification Code...'}
                  </>
                ) : (
                  <>
                    <Unlock className="w-4 h-4 mr-2" />
                    {isAdmin ? 'Admin Unlock' : 'Request Unlock'}
                  </>
                )}
              </Button>
            </>
            ...
            <>
              <div className="p-5 bg-orange-500/10 border-2 border-orange-500/20 rounded-xl">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-12 h-12 bg-orange-500/20 rounded-xl flex items-center justify-center">
                    <AlertTriangle className="w-7 h-7 text-orange-400 animate-pulse" />
                  </div>
                  <div>
                    <p className="text-white font-bold text-lg">Property Not Locked</p>
              ...
              <div className="p-4 bg-cyan-500/10 border border-cyan-500/20 rounded-lg">
                ...
                <ul className="text-xs text-gray-300 space-y-2">
                  <li className="flex items-center gap-2">
                    ...
                    Prevents new deeds or ownership transfers
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield className="w-3 h-3 text-cyan-400" />
                    Blocks unauthorized liens or mortgages
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield className="w-3 h-3 text-cyan-400" />
                    Requires your email verification to unlock
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield className="w-3 h-3 text-cyan-400" />
                    Auto-relocks after specified period
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield className="w-3 h-3 text-cyan-400" />
                    Alerts you of any bypass attempts
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield className="w-3 h-3 text-cyan-400" />
                    Validates against NYC ACRIS records
                  </li>
                </ul>
              </div>

              {/* Referral Bonus Info - Not for admins */}
              {!isAdmin && (
                <div className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <Gift className="w-5 h-5 text-purple-400" />
                    <p className="text-sm text-purple-300 font-bold">🎁 Bonus Opportunity!</p>
                  </div>
          ...
          <div className="space-y-5">
            <div className="p-5 bg-green-500/10 border-2 border-green-500/20 rounded-xl">
              <p className="text-sm text-green-300 font-semibold">
                🛡️ Title Lock will prevent any ownership changes to your property until you unlock it with email verification.
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <Select value={lockDuration} onValueChange={setLockDuration}>
                <SelectTrigger className="bg-[#0f1419] border-cyan-500/20 text-white h-12">
                  <SelectValue />
                </SelectTrigger>
              ...
              <p className="text-xs text-gray-400 bg-[#0f1419] p-3 rounded border border-cyan-500/10">
            ...
            {isAdmin && (
              <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                <p className="text-xs text-red-300">
                  <strong>👑 Admin Mode:</strong> Validation checks bypassed for admin access.
                </p>
              </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
              <Button
                ...
                variant="outline"
                className="flex-1 border-gray-500/20 text-gray-400 h-12"
              >
                Cancel
              </Button>
              <Button
                ...
                disabled={enableLockMutation.isPending}
                className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 h-12 text-base font-bold"
              >
                {enableLockMutation.isPending ? (
                  <>
                    <Lock className="w-5 h-5 mr-2 animate-spin" />
          ...
          <div className="space-y-5">
            <div className="p-5 bg-orange-500/10 border-2 border-orange-500/20 rounded-xl">
              <p className="text-sm text-orange-300 font-semibold">
                {isAdmin 
                  ? '👑 Admin access detected. Enter reason to unlock immediately.'
                  : '📧 We\'ve sent a 6-digit verification code to your email. Enter it below to unlock your property.'}
              </p>
            </div>

            <div className="space-y-3">
              <label className="text-gray-300 text-sm font-bold">Reason for Unlock:</label>
              <Input
                value={unlockReason}
                onChange={(e) => setUnlockReason(e.target.value)}
                placeholder="e.g., Refinancing my mortgage"
                className="bg-[#0f1419] border-cyan-500/20 text-white h-12"
                required
              />
            </div>
            ...
            {!isAdmin && (
              <div className="space-y-3">
                <label className="text-gray-300 text-sm font-bold">Verification Code:</label>
                <Input
                  value={unlockCode}
                  onChange={(e) => setUnlockCode(e.target.value)}
                  placeholder="Enter 6-digit code"
                  maxLength={6}
                  className="bg-[#0f1419] border-cyan-500/20 text-white text-center text-3xl tracking-widest font-mono h-16"
                  required
                />
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button
                ...
                disabled={verifyUnlockMutation.isPending || !unlockReason || (!isAdmin && !unlockCode)}
                className="flex-1 bg-orange-500 hover:bg-orange-600 h-12 text-base font-bold"
              >
