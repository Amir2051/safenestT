  ...
  return (
    <Card className="bg-gradient-to-br from-[#1a2332] to-[#0f1419] border-green-500/20">
      ...
      <CardContent className="space-y-6">
        ...
        {/* Results */}
        {result && (
          <div className={`p-6 rounded-lg border-2 ${result.isExchange ? 'bg-green-500/10 border-green-500' : 'bg-gray-500/10 border-gray-500'}`}>
            <div className="flex items-start gap-4">
              {result.isExchange ? <CheckCircle className="w-12 h-12 text-green-400 shrink-0" /> : <XCircle className="w-12 h-12 text-gray-400 shrink-0" />}
              <div className="flex-1">
                <h3 className={`text-xl font-bold mb-2 ${result.isExchange ? 'text-green-400' : 'text-gray-400'}`}>
                  {result.isExchange ? 'Exchange Activity Detected!' : 'No Exchange Activity Found'}
                </h3>
                {result.isExchange ? (
                  <div className="space-y-3">
                    <div><p className="text-gray-400 text-sm">Exchange:</p><p className="text-white font-bold text-lg">{result.exchangeName}</p></div>
                    <div><p className="text-gray-400 text-sm">Confidence:</p><Badge className="bg-green-500/20 text-green-400 border-green-500/50">{result.confidence?.toUpperCase()} {result.matchType === 'transaction' ? `(${result.matchCount} tx matches)` : '(direct address match)'}</Badge></div>
                    <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                      <p className="text-yellow-400 font-semibold mb-2">⚠️ Action Required</p>
                      <ul className="list-disc list-inside text-gray-300 text-sm space-y-1">
                        <li>Contact {result.exchangeName} compliance/fraud team</li>
                        <li>Provide deposit address & transaction hashes</li>
                        <li>Include case reference number</li>
                        <li>Request account freeze immediately</li>
                      </ul>
                    </div>
                  </div>
                ) : (
                  <p className="text-gray-300 text-sm">No transactions matched any known exchange wallet addresses in our database.</p>
                )}
              </div>
            </div>
          </div>
        ...
        {txMatches.length > 0 && (
          <div>
            <p className="text-white font-semibold mb-3 text-sm">Exchange Transaction Matches ({txMatches.length})</p>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {txMatches.map((m, i) => (
                <div key={i} className="p-3 bg-[#0f1419] rounded border border-green-500/20 text-xs flex justify-between items-center">
                  <div>
                    <Badge className="bg-green-500/20 text-green-400 border-green-500/50 mr-2">{m.exchange}</Badge>
                    <span className="text-gray-400">{m.direction}</span>
                  </div>
                  <div className="text-right">
                    <p className="text-cyan-400 font-bold">{m.value} ETH</p>
