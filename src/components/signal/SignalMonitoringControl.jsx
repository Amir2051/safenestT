...
export default function SignalMonitoringControl({ monitoringActive, healthScore, stats }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center ${
            ...
            <Radio className={`w-10 h-10 text-white ${monitoringActive ? 'animate-pulse' : ''}`} />
          </div>
          {monitoringActive && (
            <>
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">
            {monitoringActive 
              ? `Scanning cellular networks • ${stats?.total_towers_seen || 0} towers detected`
              : 'Start monitoring to detect suspicious tower activity'}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-end gap-3">
        ...
        {/* Toggle Button */}
        <Button
          onClick={() => toggleMonitoringMutation.mutate(!monitoringActive)}
