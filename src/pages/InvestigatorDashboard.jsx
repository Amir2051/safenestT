...
import { KPI, RECENT_CASES } from "@/data/localData";

export default function InvestigatorDashboard() {
  const [user, setUser] = useState(null);
  const [selectedCase, setSelectedCase] = useState(null);
  const recent = RECENT_CASES;
  ...
  const { data: myCases = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [assigned, created] = await Promise.all([
        base44.entities.MyCase.filter({ assigned_to: user.email }).catch(() => []),
        base44.entities.MyCase.filter({ created_by: user.email }).catch(() => []),
      ]);
      const map = new Map();
  ...
  const { data: tasks = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.email) return [];
      return await base44.entities.CaseTask.filter({ assigned_to: user.email, status: { $in: ["todo", "in_progress"] } }).catch(() => []);
    },
    enabled: !!user?.email,
  });
  ...
  const { data: alerts = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.email) return [];
      return await base44.entities.Notification.filter({ user_id: user?.email, read: false }).catch(() => []);
    },
    enabled: !!user?.email,
  });
    ...
    <div className="min-h-screen bg-[#000000] text-white p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {Object.entries({open_cases:["Open Cases",KPI.openCases],critical_alerts:["Critical Alerts",KPI.criticalAlerts],wallets:["Wallets Traced",KPI.walletsTraced],recovery:["Recoverability",Math.round(KPI.recoverability*100)+"%"]}).map(([k,[label,value]])=>(
            <Card key={k} className="border-white/10 bg-white/5">
                  ...
                  highPriorityCases.slice(0, 5).map(c => (
                    <div key={c.id} className="p-3 bg-[#1a2332] rounded-lg border border-gray-800 cursor-pointer hover:border-red-500/50 transition-all" onClick={() => setSelectedCase(c)}>
                      <div className="flex justify-between items-start">
                        <h4 className="font-semibold text-white text-sm">{c.case_title || c.title}</h4>
            ...
            <Card className="bg-[#0f1419] border-cyan-500/20">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <FileText className="w-5 h-5 text-cyan-400" /> Recent Investigator Cases
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="divide-y divide-white/10">
                  {recent.slice(0, 5).map(c => (
                    <div key={c.id} className="flex items-center justify-between py-3">
                      <div>
                        <div className="text-white text-sm font-medium">{c.case_number} – {c.title}</div>
                        <div className="text-gray-400 text-xs">{c.status} · Loss ${(c.loss_usd || 0).toLocaleString()}</div>
                      </div>
                      <Badge className={
                        c.priority === "critical" ? "bg-red-500/20 text-red-300 border-red-500/40" : c.priority === "high" ? "bg-orange-500/20 text-orange-300 border-orange-500/40" : "bg-blue-500/20 text-blue-300 border-blue-500/40"
                      }>{c.priority}</Badge>
                    </div>
                  ))}
                </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {activeCases.length === 0 ? (
                    ...
                    activeCases.map(c => (
                      <div key={c.id} className="p-4 bg-[#1a2332] rounded-lg border border-gray-800 hover:border-cyan-500/30 transition-all cursor-pointer flex items-center justify-between" onClick={() => setSelectedCase(c)}>
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-400 mt-1 truncate max-w-md">{c.description || "No description"}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold text-white">${(c.amount_lost || 0).toLocaleString()}</p>
